...
def ECMR2_4(question: Question) -> list[ChatSlice]:
    if question.QMR2_Spanish:
        return []

    chat_slice = ECMR2_4_step_1(question)
...
def ECMR2_4_step_1(question: Question, prompt: str = "") -> ChatSlice:
    if not prompt:
        prompt = util.translate_to(question.question, src='English', dest='French', verify=False)[1].result
